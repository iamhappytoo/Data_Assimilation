...
output_root_dir='C:/Users/BANZH/Downloads/2019spring/CEE Steve/Project_outputs/open_loop_ensemble_change_d_rz/';
%% --------- SPECIFY INPUTS HERE ----------------------------------------%%
...
tsQ=nan(20,744);
tsSrz=nan(20,745);
mapSD=nan(26,19,744,20);
mapET=nan(26,19,744,20);
mapSrz=nan(26,19,744,20);
...
restart_info.control_params.start_day=213; % start simulation
% Replace the length of the simulation and related input parameters.
restart_info.control_params.n_days=31; % number of days to simulate
...
for i_ensemble=1:N_ensemble_members
    ...
    desired_stddev= 0.5; % desired standard deviations (or coeffs. of variation if mean=1) 
...
save('C:/Users/BANZH/Downloads/2019spring/CEE steve/Project_outputs/open_loop_ensemble_change_d_rz/tsQ.mat','tsQ')
save('C:/Users/BANZH/Downloads/2019spring/CEE steve/Project_outputs/open_loop_ensemble_change_d_rz/tsET.mat','tsET')
save('C:/Users/BANZH/Downloads/2019spring/CEE steve/Project_outputs/open_loop_ensemble_change_d_rz/tsSD.mat','tsSD')
save('C:/Users/BANZH/Downloads/2019spring/CEE steve/Project_outputs/open_loop_ensemble_change_d_rz/mapET.mat','mapET')
save('C:/Users/BANZH/Downloads/2019spring/CEE steve/Project_outputs/open_loop_ensemble_change_d_rz/mapSD.mat','mapSD')
save('C:/Users/BANZH/Downloads/2019spring/CEE steve/Project_outputs/open_loop_ensemble_change_d_rz/mapSrz.mat','mapSrz')
save('C:/Users/BANZH/Downloads/2019spring/CEE steve/Project_outputs/open_loop_ensemble_change_d_rz/tsSrz.mat','tsSrz')
%% Load outputs and plot some states/fluxes
    eval(['load ' '''' restart_info.control_params.output_filename ''''])
DOY=213:1:243;
figure(1);hold on
pixSD=squeeze(mean(mapSD(14,17,:,:),4));
plot(DOY,mean(reshape(pixSD(1:744,1),24,31)),'k','LineWidth',3)
for i=1:20
    figure(1);hold on
    plot(DOY,mean(reshape(squeeze(mapSD(14,17,:,i)),24,31)),'c');
end
...
for i=1:20
    figure(2);hold on
    plot(DOY,mean(reshape(squeeze(mapSD(19,3,:,i)),24,31)),'c');
end
        xlabel('DOWY');ylabel('SD (m)')
        set(gca,'FontSize',18,'YDir','normal');grid;
        legend('Ensemble mean','Each Ensemble','Location','south')
...
for i=1:20
    figure(3);hold on
    plot(DOY,mean(reshape(squeeze(mapSD(8,10,:,i)),24,31)),'c');
end
        xlabel('DOWY');ylabel('SD (m)')
        set(gca,'FontSize',18,'YDir','normal');grid;
        legend('Ensemble mean','Each Ensemble','Location','south')
...
plot(DOY,mean(reshape(pixSrz(1:744,1),24,31)),'k','LineWidth',3)
for i=1:20
    figure(4);hold on
    plot(DOY,mean(reshape(squeeze(mapSrz(14,17,:,i)),24,31)),'c');
end
...
for i=1:20
    figure(5);hold on
    plot(DOY,mean(reshape(squeeze(mapSrz(19,3,:,i)),24,31)),'c');
end
        xlabel('DOWY');ylabel('Srz (m)')
        set(gca,'FontSize',18,'YDir','normal');grid;
        legend('Ensemble mean','Each Ensemble','Location','south')
...
for i=1:20
    figure(6);hold on
    plot(DOY,mean(reshape(squeeze(mapSrz(8,10,:,i)),24,31)),'c');
end
        xlabel('DOWY');ylabel('Srz (m)')
        set(gca,'FontSize',18,'YDir','normal');grid;
        legend('Ensemble mean','Each Ensemble','Location','south')
...
plot(DOY,mean(reshape(pixET(1:744,1),24,31)),'k','LineWidth',3)
for i=1:20
    figure(7);hold on
    plot(DOY,mean(reshape(squeeze(mapET(14,17,:,i)),24,31)),'c');
end
        xlabel('DOWY');ylabel('ET (m)')
        set(gca,'FontSize',18,'YDir','normal');grid;
        legend('Ensemble mean','Each Ensemble','Location','south')
figure(8);hold on
...
for i=1:20
    figure(8);hold on
    plot(DOY,mean(reshape(squeeze(mapET(19,3,:,i)),24,31)),'c');
end
        xlabel('DOWY');ylabel('ET (m)')
        set(gca,'FontSize',18,'YDir','normal');grid;
        legend('Ensemble mean','Each Ensemble','Location','south')
...
for i=1:20
    figure(9);hold on
    plot(DOY,mean(reshape(squeeze(mapET(8,10,:,i)),24,31)),'c');
end
        xlabel('DOWY');ylabel('ET (m)')
        set(gca,'FontSize',18,'YDir','normal');grid;
        legend('Ensemble mean','Each Ensemble','Location','south')
